...
- **`landscape.html`** — the rendered HTML artefact, regenerated by
  `scripts/render.py` on every `make build`. ~912 entries across ~30
  sections / sub-groups, with tier badges (T1 battle-tested → T5
  theoretical / informal). Under Path A (since #68, 2026-05-18) this
  file is build output — edit `data/landscape.json` instead.
...
plus a render-from-JSON script in `scripts/render.py`. As of #68
(2026-05-18), **`data/landscape.json` is the source of authority** and
`landscape.html` is a rendered artefact regenerated by `scripts/render.py`
on every `make build`. This is Path A — see `docs/DECISIONS.md`
2026-05-18 Path A entry for the rationale and the round-trip evidence.

Workflow (Path A):

1. Edit `data/landscape.json` directly — or, more commonly, let the
   intake-research / section-audit bots open a PR with structured cell
   edits. The PR gets a rendered-cell preview comment from
   `.github/workflows/diff-preview.yml` so reviewers see what each
   touched row will look like in the HTML after merge.
2. Run `make build` locally to reconcile JSON, rebuild edges + citation
   trajectories, and re-render `landscape.html` as the artefact.
3. Run `make validate` (described below). Fix any reported failures.
4. Commit `data/landscape.json`, the regenerated `data/landscape.edges.json`,
   and the regenerated `landscape.html` together. CI re-runs the gates
   (`.github/workflows/validate.yml`) plus the Path A byte-identity
   check (`make build-path-a`) on push and PR.
...
`make validate` runs five cheap correctness gates in ~25 seconds. None of
...
| 5 | Tier-provenance + freshness     | Cells whose tier disagrees with their citation evidence, or rows that fail SCHEMA.md §3a/§3b/§7.1.12. |

`make build-path-a` is a separate gate (also in CI) that asserts
`render.py(data/landscape.json)` is byte-identical to the committed
`landscape.html`. This is the property Path A depends on: the JSON
source must round-trip cleanly to the HTML artefact.
...
CI runs `make validate` and `make build-path-a` on push to `main` and on
