...
is now done. `data/landscape.json` is the source of authority;
`landscape.html` is a rendered artefact regenerated by `scripts/render.py`
on every `make build`. All five writer scripts (`bucket_s2_citations.py`,
`fetch_commit_trajectories.py`, `fetch_download_trajectories.py`,
`apply_intake_pr.py`, `apply_audit_pr.py`, `research_decay_causes.py`)
write JSON via the `scripts/_cell_writer.py` helper. The legacy
`landscape.html` write paths remain behind a `--target landscape.html`
flag that emits a `DeprecationWarning`, so a rollback is possible if
something downstream breaks.

The Path A byte-identity gate (`make build-path-a`) asserts that
`render.py(data/landscape.json)` is byte-identical to the committed
`landscape.html`. It runs in `.github/workflows/validate.yml` alongside
the five `make validate` gates. To restore the visual-context review
experience that Path B got from HTML diffs,
`.github/workflows/diff-preview.yml` posts a rendered-cell preview
comment on every PR that touches `data/landscape.json`, rendering each
touched record's full row with changed cells flagged.

**Why.** The earlier Path A entry committed to the inversion; this
entry records that the execution is done. The review-surface concern
(reviewers losing context when changes become structured JSON deltas
instead of in-context HTML edits) is resolved by the diff-preview
workflow. The deprecation flag preserves a rollback path for ~one
release cycle while we watch the auto-research and section-audit bots
operate under the new contract.

**Reversal cost.** Low for ~one release cycle (writer scripts still
have the HTML path behind a flag). After that, removing the HTML write
paths is a small mechanical task — at which point the only way back to
Path B would be reverting the writer-flip commits.
